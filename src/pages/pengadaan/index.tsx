...
const EmptyAuctionState = () => (
	<div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-16 text-center">
		<img src="/lelang.svg" alt="Pengadaan" className="mb-4 h-44 w-44" />
		<p className="text-base text-gray-600">Pengadaan belum tersedia saat ini.</p>
	</div>
);
							...
							<div className="mb-4 flex items-center justify-between">
								<div>
									...
									<p className="mt-1 text-sm text-gray-500">
										Total {total} data pengadaan tersedia
									</p>
								</div>
							</div>
							...
							{totalPage > 1 ? (
								<div className="mt-6 flex items-center justify-between gap-4 border-t border-gray-200 pt-6">
									...
									<div className="flex items-center gap-2">
										{currentPage > 1 ? (
											<Link
												href={`/pengadaan?page=${currentPage - 1}`}
...
export const getServerSideProps: GetServerSideProps<PengadaanPageProps> = async (
	context,
) => {
